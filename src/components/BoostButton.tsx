...
export default function BoostButton({ postId, postType, onBoostSuccess }: BoostButtonProps) {
  ...
  return (
    <button
      onClick={handleBoost}
      disabled={isBoosting}
      className="bg-gradient-to-r from-yellow-400 to-orange-500 text-white px-4 py-2 rounded-lg font-semibold hover:from-yellow-500 hover:to-orange-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-md hover:shadow-lg flex items-center gap-2 text-sm"
    >
      {isBoosting ? (
        <>
