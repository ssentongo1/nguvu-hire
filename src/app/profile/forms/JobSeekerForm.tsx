...
export default function JobSeekerForm({ profile, onSave }: any) {
  ...
  const handleFileChange = async (e: any) => {
    try {
      ...
      const filePath = `${user.id}/jobseeker-${Date.now()}.${file.name.split(".").pop()}`;
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (error) {
        ...
        alert("Job Seeker profile saved successfully!");
        onSave(data);
      }
    } catch (error) {
      console.error("Save error:", error);
  ...
  return (
    <form onSubmit={handleSubmit} className={cardClass}>
      <div className="text-center mb-6">
        <h2 className={`text-xl sm:text-2xl font-bold mb-2 ${darkMode ? "text-white" : "text-gray-900"}`}>Job Seeker Profile</h2>
        <p className={`text-xs sm:text-sm ${darkMode ? "text-gray-300" : "text-gray-600"}`}>Complete your profile to stand out to employers</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="text-center">
          <div className="relative inline-block">
            {formData.profile_picture_url ? (
              ...
              <div className={`w-24 h-24 sm:w-32 sm:h-32 rounded-xl border-4 border-white shadow-lg flex items-center justify-center ${
                darkMode ? "bg-gray-800" : "bg-gradient-to-br from-indigo-100 to-blue-100"
              }`}>
          ...
          {uploading && <p className={`text-xs mt-1 ${darkMode ? "text-indigo-400" : "text-indigo-600"}`}>Uploading photo...</p>}
        </div>
